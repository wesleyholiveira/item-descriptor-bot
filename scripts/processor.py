...
data = {}
language = os.getenv("DESCRIPTION_LANGUAGE", "pt-br")
...
def main():
    try:
        ...
        path, is_zuka = get_path_cli(sys.argv, data_raw_path)

        r = redis.Redis(host=host, port=port, decode_responses=True)
        pipe = r.pipeline()
        schema = (TextField("$.title", as_name="title"),)

        logger.info("Criando indice 'items'")
        r.ft(f"items").create_index(
            schema,
            definition=IndexDefinition(prefix=["items:"], index_type=IndexType.JSON),
        )

        path_list = [data_raw_path + "/en"]
        if is_zuka is True:
            path_list.append(path)

        for data_path in path_list:
            logger.info(f"Lendo o diretório: {data_path}")
            for file in os.listdir(data_path):
                full_path = f"{data_path}/{file.lower()}"
                    ...
                    for item in items:
                        for group in item.groups():
                            ...
                            if (
                                ...
                                logger.info(f"Criando o indice 'items:{item_category}'")
                                pipe.ft(f"items:{item_category}").create_index(
                                    schema,
                                    definition=IndexDefinition(
                                        prefix=[f"items:{item_category}:"],
                                ...
                                for value in data[item_category].values():
                                    ...
                                    pipe.json().set(
                                        f"items:{item_category}:{redis_key}",
                                        Path.root_path(),
                                        value,
                                    )
